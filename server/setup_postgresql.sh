...
sudo PGPASSWORD=$PG_PASSWORD psql -U $PG_USER -h $PG_HOST -p $PG_PORT -d $PG_DATABASE -v "ON_ERROR_STOP=1" --set=sslmode=require -f ./db/039-remove-fsm-and-door-core-id.sql
sudo PGPASSWORD=$PG_PASSWORD psql -U $PG_USER -h $PG_HOST -p $PG_PORT -d $PG_DATABASE -v "ON_ERROR_STOP=1" --set=sslmode=require -f ./db/040-alter-locations.sql
