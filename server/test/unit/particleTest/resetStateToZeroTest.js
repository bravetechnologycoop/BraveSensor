...
describe('particle.js unit tests: resetStateToZero', () => {
  beforeEach(() => {
  ...
  describe('given a Particle Device ID', () => {
    beforeEach(async () => {
      ...
      await particle.resetStateToZero(this.deviceId)
    })
  ...
  describe('if the cloud function returns unsuccessful response', () => {
    ...
    it('should throw an error with appropriate message', async () => {
      try {
        ...
        expect(error.message).to.equal(`resetStateToZero: Error calling function for device with particleDeviceId: ${this.deviceId}`)
      }
    })
  })
})
