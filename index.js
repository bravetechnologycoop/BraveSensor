...
const locationsDashboardTemplate = fs.readFileSync(
  `${__dirname}/locationsDashboard.mst`,
  'utf-8'
)
...
app.get('/dashboard/:locationId', async (req, res) => {
  ...
  try {
    ...
    for (const recentSession of recentSessions) {
      const startTime = moment(recentSession.startTime, moment.ISO_8601)
...
smartapp
  ...
  .page('mainPage', (context, page, configData) => {
    // eslint-disable-line no-unused-vars -- configData might be needed in the future
  ...
  .updated((context, updateData) => {
    ...
    context.api.subscriptions.unsubscribeAll().then(() => {
      ...
      context.api.subscriptions.subscribeToDevices(
        ...
        'battery',
        'battery',
        'myBatteryEventHandler'
      )
      context.api.subscriptions.subscribeToDevices(
        context.config.contactSensor,
        'temperatureMeasurement',
        'temperature',
        'myTemperatureEventHandler'
      )
      context.api.subscriptions.subscribeToDevices(
        context.config.motionSensor,
        'motionSensor',
        'motion',
        'myMotionEventHandler'
      )
      context.api.subscriptions.subscribeToDevices(
  ...
  .subscribedEventHandler('myContactEventHandler', (context, deviceEvent) => {
    ...
    const LocationID =
      context.event.eventData.installedApp.config.LocationID[0].stringConfig
        .value
    const DeviceID =
      context.event.eventData.installedApp.config.DeviceID[0].stringConfig.value
...
async function autoResetSession(locationid) {
  try {
    ...
    const session = await db.getMostRecentSession(locationid, client)
    await db.closeSession(session.sessionid, client)
    await db.updateSessionResetDetails(
      session.sessionid,
      'Auto reset',
      'Reset',
      client
...
if (!helpers.isTestEnvironment()) {
  setInterval(async () => {
    ...
    for (let i = 0; i < locations.length; i++) {
      ...
      const stateHistoryQuery = await redis.getStatesWindow(
        currentLocationId,
        '+',
        '-',
        60
      ...
      for (let i = 1; i < stateHistoryQuery.length; i++) {
        if (
          stateHistoryQuery[i].state == STATE.RESET &&
          !(
            stateHistoryQuery[i - 1].state == STATE.NO_PRESENCE_NO_SESSION ||
            stateHistoryQuery[i - 1].state == STATE.RESET
          ) &&
          !resetDiscrepancies.includes(stateHistoryQuery[i].timestamp)
        ) {
          console.log(
            `The Reset state logged at ${stateHistoryQuery[i].timestamp} has a discrepancy`
          )
          resetDiscrepancies.push(stateHistoryQuery[i].timestamp)
          console.log(
            'Adding a reset state to the sessions table since there seems to be a discrepancy'
          )
...
app.post(
  '/api/st',
  Validator.body(['lifecycle']).exists(),
  (req, res, next) => {
    ...
    try {
      ...
      if (validationErrors.isEmpty()) {
        ...
        helpers.log(
          `Bad request, parameters missing ${JSON.stringify(validationErrors)}`
        )
        res.status(400).send()
      }
    } catch (err) {
...
app.post(
  '/api/xethru',
  Validator.body(['locationid']).exists(),
  async (req, res) => {
    try {
      const validationErrors = Validator.validationResult(req)
...
async function handleSensorRequest(currentLocationId) {
  ...
  if (currentState != prevState) {
    ...
    // Checks if current state belongs to the session triggerStates
      ...
      const client = await db.beginTransaction()
      const latestSession = await db.getMostRecentSession(
        currentLocationId,
        client
      )
      ...
      if (typeof latestSession !== 'undefined') {
        ...
        if (latestSession.end_time == null) {
          ...
          const currentSession = await db.updateSessionState(
    ...
    if (typeof currentSession !== 'undefined') {
      if (
        currentSession.state == 'Still' ||
        currentSession.state == 'Breathing'
      ) {
        if (currentSession.end_time == null) {
          // Only increases counter if session is open
          await db.updateSessionStillCounter(
            currentSession.still_counter + 1,
...
async function sendInitialChatbotMessage(session) {
  ...
  const alertReason = session.alert_reason
  ...
  setTimeout(
    fallbackMessage,
    locationData.unresponded_session_timer,
    session.sessionid
  )
}
...
async function reminderMessage(sessionid) {
  if (!helpers.isTestEnvironment()) {
    ...
    const session = await db.getSessionWithSessionId(sessionid) // Gets the updated state for the chatbot
    if (session.chatbot_state == STATE.STARTED) {
      ...
      await sendTwilioMessage(
        locationData.twilio_number,
        session.phonenumber,
        `This is a reminder to check on the bathroom`
      )
...
async function checkHeartbeat(locationid) {
  ...
  if (
    XeThruDelayMillis > XETHRU_THRESHOLD_MILLIS &&
    !location.xethru_sent_alerts
  ) {
    console.log(
      `XeThru Heartbeat threshold exceeded; sending alerts for ${location.locationid}`
    )
...
app.post('/sms', async (req, res) => {
  ...
  const session = await db.getMostRecentSessionPhone(to, client)
  const chatbot = new Chatbot(
    session.sessionid,
    session.locationid,
    session.chatbot_state,
