...
let TRIGGERSTATES = [STATE.DOOR_CLOSED_START, STATE.MOTION_DETECTED]

// These states will close an ongoing session for a certain location
let CLOSINGSTATES = [STATE.DOOR_OPENED_CLOSE, STATE.COMPLETED]

// These states will start a chatbot session for a location
let CHATBOTSTARTSTATES = [STATE.SUSPECTED_OD]
...
app.get('/dashboard/:locationId', async (req, res) => {
  ...
  try {
    ...
    for (const recentSession of recentSessions) {
      let startTime = moment(recentSession.startTime, moment.ISO_8601).tz('America/Vancouver').format('DD MMM Y, hh:mm:ss A')
    ...
    for (let i = 0; i < locations.length; i++) {
      ...
      for (let i = 1; i < stateHistoryQuery.length; i++) {
        if (
          ...
          !(stateHistoryQuery[i - 1].state == STATE.NO_PRESENCE_NO_SESSION || stateHistoryQuery[i - 1].state == STATE.RESET) &&
