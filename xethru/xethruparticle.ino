...
void setup() 
{
  ...
  // Set LED control
  set_led_control(0); // 0: OFF; 1: SIMPLE; 2: FULL

  // Set detection zone
  set_detection_zone(0.4, 4.0); // First variable = Lower limit, Second variable = Upper limit
  ...
  // For every loop we check to see if we have received any respiration data
    ...
    if(get_respiration_data(&msg)) {
        ...
        if(msg.state_code != XTS_VAL_RESP_STATE_INITIALIZING || msg.state_code != XTS_VAL_RESP_STATE_ERROR || msg.state_code != XTS_VAL_RESP_STATE_UNKNOWN) {
            ...
            if(msg.movement_fast >= (1 /*some value*/)) {
                // some large movments
                // Check the movement_fast as well
                 
                // There is definitely movement.
                
            }
            else if(msg.movement_slow >= 1/*some other value*/ && msg.rpm != 0) {
                // Likely there is some movement
                // If already in breathing state, then the RPM should be able to provide enough information
                
                // Can assume they're moving
            }
            else if(msg.rpm != 0) {
                // No movement and no breathing detected. Could be a trigger or criteria for a suspected overdose?
        ...
        if(i%30 == 0) {
            //Check the battery level every so often
            float cellVoltage = batteryMonitor.getVCell(); // Voltage of the LiPo battery
            float stateOfCharge = batteryMonitor.getSoC(); // Percentage of battery remaining in the LiPo battery
            
            if(prevSOC - stateOfCharge > 0 || stateOfCharge == 100) {
                charging = TRUE;
            }
            else if(prevSOC - stateOfCharge < 0) {
                charging = FALSE;
            }
            
            prevSOC = stateOfCharge;
        
            if(charging) {
                Particle.publish("source", "USB");
            }
            else if(!charging) {
                Particle.publish("source", "LiPo Battery");
                if(stateOfCharge <= 20)
                   errorPublish("Low Battery, 20% remaining");
            }
        }
    }
//  delay(1000);
...
//	snprintf(buf, sizeof(buf), "{\"a\":%d, \"b\":%s}", data, *data);
	snprintf(buf, sizeof(buf), "{\"devicetype\":\"XeThru\", \"location\":\"BraveOffice\", \"device\":\"1\", \"distance\":\"%s\", \"rpm\":\"%s\", \"slow\":\"%s\", \"fast\":\"%s\", \"state\":\"%s\"}", distance, rpm, slow, fast, x_state);
...
// Set Debug Level
void set_led_control(int control) 
{
  // For byte index 2
  // 0: OFF
  // 1: SIMPLE
...
void configure_noisemap() 
{
  ...
  send_buf[3] = 0x02; // 0x06: Use default noisemap and adaptive noisemap
  send_buf[4] = 0x00;
  send_buf[5] = 0x00;
  send_buf[6] = 0x00;
