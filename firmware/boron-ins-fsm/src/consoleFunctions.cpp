...
int im21_door_id_set(String command) { 
  ...
  if(*checkForEcho == 'e'){  
    EEPROM.get(ADDR_IM_DOORID, doorIDHolder.byte3);  
    EEPROM.get((ADDR_IM_DOORID + 1), doorIDHolder.byte2);  
    EEPROM.get((ADDR_IM_DOORID + 2), doorIDHolder.byte1);  

    snprintf(buffer, sizeof(buffer), "{\"byte1\":\"%02X\", \"byte2\":\"%02X\", \"byte3\":\"%02X\"}", 
            doorIDHolder.byte3, doorIDHolder.byte2, doorIDHolder.byte1); 
  ...
  else // else not echo, so we have a new door ID to parse
  {
    ...
    byteholder3 = strdup(command.substring(0, split1).c_str());       // get first byte of input and copy to holder variable
    globalDoorID.byte3 = (uint8_t)strtol(byteholder3, NULL, 16);      // convert it to hex and set the third byte of the door ID
    ...
    byteholder1 = strdup(command.substring(split2 + 1, split3).c_str());
    globalDoorID.byte1 = (uint8_t)strtol(byteholder1, NULL, 16);

    //write new global door ID to flash
    EEPROM.put(ADDR_IM_DOORID, globalDoorID.byte3);
    EEPROM.put((ADDR_IM_DOORID + 1), globalDoorID.byte2);
    EEPROM.put((ADDR_IM_DOORID + 2), globalDoorID.byte1);
