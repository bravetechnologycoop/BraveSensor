...
>>>>>>> 779f92df87eb9ef6fef4af4f56c80ed4f493b3bb
        ...
        // string.toInt() returns 0 if it fails, so can't distinguish between
        // user entering 0 vs entering bad input. So convert to char and use
        // ascii table
...
>>>>>>> 779f92df87eb9ef6fef4af4f56c80ed4f493b3bb
    ...
    // returns initial timer length if valid input is given, otherwise returns
    // -1
    ...
    // returns stillness timer length if valid input is given, otherwise returns
    // -1
    int stillness_timer_set(String input) {
        ...
        // if e, echo the current threshold
        if (*holder == 'e') {
            EEPROM.get(ADDR_STATE3_MAX_STILLNES_TIME,
    ...
    int im21_door_id_set(String command) {  // command is a long string with all
                                            // the config values
...
>>>>>>> 779f92df87eb9ef6fef4af4f56c80ed4f493b3bb
    ...
    int force_reset(String command) {
        ...
        if (*(holder + 1) != 0) {
            ...
            returnFlag = 1;
            bool msg_sent = Particle.publish(
                "YOU SHALL NOT PANIC!!",
