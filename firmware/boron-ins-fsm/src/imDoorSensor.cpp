...
doorData checkIM()
{
    ...
    static doorData returnDoorData = {INITIAL_DOOR_STATUS, INITIAL_DOOR_STATUS,
                                      0};

    // BLE scanner is set fast enough to load duplicate advertising data packets
    // Every time the IM Door Sensor transmits a door event, filter out the
    // duplicates and publish
    if (os_queue_take(bleQueue, &currentDoorData, 0, 0) == 0) {
        // Log.warn("raw door sensor output - control:  prev, current: 0x%02X,
        // 0x%02X", previousDoorData.controlByte, currentDoorData.controlByte);
        // Log.warn("raw door sensor output - data byte prev, current: 0x%02X,
        // 0x%02X", previousDoorData.doorStatus, currentDoorData.doorStatus);

        // Checks if the 2nd bit (counting from 0) of doorStatus is 1
        // read as: doorLowBatteryFlag is true if doorStatus AND 0b0100 is not
        // 0b0000
        doorLowBatteryFlag = (currentDoorData.doorStatus & (1 << 2)) != 0;

        // After a certain thereshold, the next door message received will
        // trigger a boron heartbeat
        ...
        // if this is the first door event received after firmware bootup,
        // publish
        ...
        else if (currentDoorData.controlByte ==
                 (previousDoorData.controlByte + 0x01)) {
            returnDoorData = currentDoorData;
            previousDoorData = currentDoorData;
        }
        // if curr > prev + 1, missed an event, publish warning
                 ...
                 (previousDoorData.controlByte + 0x01)) {
            Log.error(
                "curr > prev + 1, WARNING WARNING WARNING, missed door event!");
            missedDoorEventCount++;
            logAndPublishDoorWarning(previousDoorData, currentDoorData);
            returnDoorData = currentDoorData;
            previousDoorData = currentDoorData;
        }
        // special case for when control byte rolls over from FF to 00, don't
        // want to lose event or publish missed door event warning
        else if ((currentDoorData.controlByte == 0x00) &&
                 (previousDoorData.controlByte == 0xFF)) {
            returnDoorData = currentDoorData;
            previousDoorData = currentDoorData;
        }
        else {
        ...
        // record the time this value was pulled from the queue and control byte
        // was checked
...
void logAndPublishDoorData(doorData previousDoorData, doorData currentDoorData)
{
    ...
    sprintf(doorPublishBuffer,
            "{ \"deviceid\": \"%02X:%02X:%02X\", \"data\": \"%02X\", "
            "\"control\": \"%02X\" }",
            globalDoorID.byte1, globalDoorID.byte2, globalDoorID.byte3,
            currentDoorData.doorStatus, currentDoorData.controlByte);
    Particle.publish("IM Door Sensor Data", doorPublishBuffer, PRIVATE);
    Log.warn("published, 0x%02X", currentDoorData.controlByte);
}

void logAndPublishDoorWarning(doorData previousDoorData,
                              doorData currentDoorData)
{
    char doorPublishBuffer[128];

    sprintf(doorPublishBuffer,
            "{ \"deviceid\": \"%02X:%02X:%02X\", \"prev_control_byte\": "
            "\"%02X\", \"curr_control_byte\": \"%02X\" }",
            globalDoorID.byte1, globalDoorID.byte2, globalDoorID.byte3,
            previousDoorData.controlByte, currentDoorData.controlByte);
    Particle.publish("IM Door Sensor Warning", doorPublishBuffer, PRIVATE);
    Log.warn(
        "Published IM Door Sensor warning, prev door byte = 0x%02X, curr door "
        "byte = 0x%02X",
        previousDoorData.controlByte, currentDoorData.controlByte);
}
...
void threadBLEScanner(void *param)
{
    ...
    while (true) {
        // filter has to be remade each time, as globalDoorId (from eeprom) is
        // usually read while this thread is already in while loop
        BleScanFilter filter;
        char address[18];
        // add device IDs for IM21 to the filter
        sprintf(address, "B8:7C:6F:%02X:%02X:%02X", globalDoorID.byte3,
                globalDoorID.byte2, globalDoorID.byte1);
        filter.deviceName("iSensor ").address(address);
        sprintf(address, "8C:9A:22:%02X:%02X:%02X", globalDoorID.byte3,
                globalDoorID.byte2, globalDoorID.byte1);
        filter.address(address);
        // add device ID for IM24 to the filter
        sprintf(address, "AC:9A:22:%02X:%02X:%02X", globalDoorID.byte3,
                globalDoorID.byte2, globalDoorID.byte1);
        filter.address(address);
        ...
        for (BleScanResult scanResult : scanResults) {
            ...
            // dooradvertisingdata[1-3] (3 bytes): Last 3 bytes of door sensor
            // address dooradvertisingdata[4] (1 byte): Type ID (door sensor,
            // thermometer, smoke detector, etc.) dooradvertisingdata[5] (1
            // byte): Event Data bit[3]: heartbeat? bit[2]: low battery? bit[1]:
            // door open? bit[0]: tamper alarm?
            // dooradvertisingdata[6] (1 byte): Control Data
            // More info:
            // https://drive.google.com/file/d/1ZbnHi7uA_xMWVIiMbQjZlbT3OOoykbr4/view?usp=sharing
            scanResult.advertisingData().get(
                BleAdvertisingDataType::MANUFACTURER_SPECIFIC_DATA,
                doorAdvertisingData, BLE_MAX_ADV_DATA_LEN);
            ...
            if ((scanThreadDoorData.doorStatus & (1 << 3)) != 0 &&
                stateMachineDebugFlag) {  //
                // from particle docs, max length of publish is 622 chars, I am
                // assuming this includes null char
                char debugMessage[622] = "";
                for (int i = 0; i < BLE_MAX_ADV_DATA_LEN; i++) {
                    snprintf(debugMessage + strlen(debugMessage),
                             sizeof(debugMessage), "%02X ",
                             doorAdvertisingData[i]);
                }
                Particle.publish("Door Heartbeat Received", debugMessage,
                                 PRIVATE);
            }
...
//
// Parameters: The IM door sensor door status (byte 5 of the door sensor
// advertising data). Returns: 1 if the door is open, 0 if the door is closed.
...
// The door status is considered unknown when it is equal to
// INITIAL_DOOR_STATUS, which occurs upon initial startup.
//
// Parameters: The IM door sensor door status (byte 5 of the door sensor
// advertising data). Returns: 1 if the door status is unknown, 0 if the door
// status is known.
