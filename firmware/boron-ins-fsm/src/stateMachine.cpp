...
void getHeartbeat(){
    ...
    // 3rd "if condition" is a heartbeat hasnt been published in the lasts SM_HEARTBEAT_INTERVAL
    if(lastHeartbeatPublish == 0 || (millis() - lastHeartbeatPublish) > SM_HEARTBEAT_INTERVAL || (doorMessageReceivedFlag && millis() - doorHeartbeatReceived >= HEARTBEAT_PUBLISH_DELAY)){
      //from particle docs, max length of publish is 622 chars, I am assuming this includes null char
