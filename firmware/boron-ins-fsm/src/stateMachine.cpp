...
void state0_idle()
{
    ...
    if (((unsigned long)checkINS.iAverage > ins_threshold) &&
        ...
        Log.warn(
            "In state 0, door closed and seeing movement, heading to state 1");
...
void state1_15sCountdown()
{
    ...
    Log.info(
        "You are in state 1, 15s countdown: Door status, iAverage, timer = "
        "0x%02X, %f, %ld",
        checkDoor.doorStatus, checkINS.iAverage, (millis() - state1_timer));
    publishDebugMessage(1, checkDoor.doorStatus, checkINS.iAverage,
                        (millis() - state1_timer));

    // fix outputs and state exit conditions accordingly
    if ((unsigned long)checkINS.iAverage > 0 &&
        ...
        Log.warn(
            "door closed && motion for > Xs, going to state 2 from state1");
...
void state2_duration()
{
    ...
    Log.info(
        "You are in state 2, duration: Door status, iAverage, timer = 0x%02X, "
        "%f, %ld",
        checkDoor.doorStatus, checkINS.iAverage,
        (millis() - state2_duration_timer));
    publishDebugMessage(2, checkDoor.doorStatus, checkINS.iAverage,
                        (millis() - state2_duration_timer));
    ...
    if ((unsigned long)checkINS.iAverage > 0 &&
        (unsigned long)checkINS.iAverage < ins_threshold) {
        Log.warn(
            "Seeing stillness, going to state3_stillness from state2_duration");
        ...
        Log.warn(
            "Door opened, session over, going to idle from state2_duration");
        ...
        Log.warn(
            "See duration alert, going from state2_duration to idle after "
            "alert "
            "publish");
        publishStateTransition(2, 0, checkDoor.doorStatus, checkINS.iAverage);
        saveStateChange(2, 4);
...
void state3_stillness()
{
    ...
    Log.info(
        "You are in state 3, stillness: Door status, iAverage, timer = 0x%02X, "
        "%f, %ld",
        checkDoor.doorStatus, checkINS.iAverage,
        (millis() - state3_stillness_timer));
    publishDebugMessage(3, checkDoor.doorStatus, checkINS.iAverage,
                        (millis() - state3_stillness_timer));

    // fix outputs and state exit conditions accordingly
    if ((unsigned long)checkINS.iAverage > ins_threshold) {
        Log.warn(
            "motion spotted again, going from state3_stillness to "
            "state2_duration");
        ...
        Log.warn(
            "door opened, session over, going from state3_stillness to idle");
        ...
        Log.warn(
            "See duration alert, going from state3 to idle after alert "
            "publish");
        publishStateTransition(3, 0, checkDoor.doorStatus, checkINS.iAverage);
        saveStateChange(3, 4);
        Log.error("Duration Alert!!");
...
void publishStateTransition(int prevState, int nextState,
                            unsigned char doorStatus, float INSValue)
{
    if (stateMachineDebugFlag) {
        ...
        snprintf(stateTransition, sizeof(stateTransition),
                 "{\"prev_state\":\"%d\", \"next_state\":\"%d\", "
                 "\"door_status\":\"0x%02X\", \"INS_val\":\"%f\"}",
                 prevState, nextState, doorStatus, INSValue);
        Particle.publish("State Transition", stateTransition, PRIVATE);
    }
}

void publishDebugMessage(int state, unsigned char doorStatus, float INSValue,
                         unsigned long timer)
{
    static unsigned long lastDebugPublish = 0;

    if (stateMachineDebugFlag &&
        (millis() - lastDebugPublish) > DEBUG_PUBLISH_INTERVAL) {
        // from particle docs, max length of publish is 622 chars, I am assuming
        // this includes null char
        char debugMessage[622];
        snprintf(debugMessage, sizeof(debugMessage),
                 "{\"state\":\"%d\", \"door_status\":\"0x%02X\", "
                 "\"INS_val\":\"%f\", \"timer_status\":\"%ld\"}",
...
/**
 * Saves the state transition data onto queues, including the last state, reason
 *of transition and time spent in state.
 *
 * Reason Code | Meaning
 * -------------------------------
...
void getHeartbeat()
{
    ...
    //     publish to 1 instead of 3 because the IM Door Sensor broadcasts 3
    //     messages The doorMessageReceivedFlag is set to true when any IM Door
    //     Sensor message is received, but only after a certain threshold
    // 3rd "if condition" is true only if a heartbeat hasnt been published in
    // the last SM_HEARTBEAT_INTERVAL
    if (lastHeartbeatPublish == 0 ||
        ...
        // logs time in milliseconds since last IM Door Sensor message was
        // received the particle name is a bit misleading, remains this way
        // because the server uses "doorLastHeartbeat" to check if the IM Door
        // Sensor sensor has disconnected
        ...
        // logs each state, reason of transitioning away, and time spent in
        // state (ms)
        writer.name("states").beginArray();
        int numStates = stateQueue.size();
        for (int i = 0; i < numStates; i++) {
            // If heartbeat message is near full, break, report rest of states
            // in next heartbeat
            if (writer.dataSize() >= HEARTBEAT_STATES_CUTOFF) {
                Log.warn(
                    "Heartbeat message full, remaining states will be reported "
                    "next heartbeat");
                break;
            }
            writer.beginArray()
