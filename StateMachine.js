...
class StateMachine {
  ...
  async getNextState(db, redis) {
    ...
    if (states == undefined) {
      ...
      state = states.state // Takes current state in case none of the state conditions get met for a state transition
      ...
      switch (states.state) {
        ...
        case STATE.NO_PRESENCE_NO_SESSION:
          ...
          const mov_f_avg = xethru_history.map(entry => entry.mov_f).reduce((a, b) => a + b) / xethru_history.length
          // eslint-disable-next-line no-case-declarations
          const mov_s_avg = xethru_history.map(entry => entry.mov_s).reduce((a, b) => a + b) / xethru_history.length
