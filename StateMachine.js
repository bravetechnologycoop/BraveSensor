...
class StateMachine {
  ...
  async getNextState(db, redis) {
    ...
    const xethru_history = await redis.getXethruWindow(
      this.location,
      '+',
      '-',
      15
    ) // Array of last 15 readings from this location
    ...
    if (states == undefined) {
      ...
      switch (states.state) {
        ...
        case STATE.NO_PRESENCE_NO_SESSION:
          ...
          var mov_f_avg =
            xethru_history.map(entry => entry.mov_f).reduce((a, b) => a + b) /
            xethru_history.length
          var mov_s_avg =
            xethru_history.map(entry => entry.mov_s).reduce((a, b) => a + b) /
            xethru_history.length
          ...
          // Removing the condition for motion sensor to be active to go into this loop since we don't have a motion sensor in this space
            ...
            (mov_f_avg > residual_mov_f || mov_s_avg > 55) &&
        ...
        case STATE.MOVEMENT:
          ...
          if (
            session.od_flag == OD_FLAG_STATE.NO_OVERDOSE &&
            (await db.isOverdoseSuspected(xethru, session, location_data))
          ) {
