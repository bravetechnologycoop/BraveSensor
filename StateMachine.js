    ...
    if (states == undefined) {
      ...
      switch (states.state) {
        ...
        case STATE.NO_PRESENCE_NO_SESSION:
          ...
          var mov_f_avg = xethru_history.map(entry => entry.mov_f).reduce((a, b) => a + b) / xethru_history.length
          var mov_s_avg = xethru_history.map(entry => entry.mov_s).reduce((a, b) => a + b) / xethru_history.length
